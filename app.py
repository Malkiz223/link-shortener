...
def index_post():
    ...
    has_dot_regexp = r'\w+[.]\w\w+'  # имеет символ, точку и минимум два символа после точки
    has_any_spaces_regexp = r'\s+'  # имеет пробельные символы в ссылке
    has_dot_and_two_characters_after = re.search(has_dot_regexp, long_url)
    has_any_spaces = re.search(has_any_spaces_regexp, long_url)
    if not has_dot_and_two_characters_after or has_any_spaces:
...
def redirect_to_long_url(short_url):
    """
    Функция идёт в базу и ищет там комбинацию, которая пришла от пользователя.
